...
let print_instr fmt = function
  ...
  | Econst (Cstring n, r, l) -> fprintf fmt "mov some_string %a;  goto %a" Register.print r Label.print l
  ...
  | Embbranch (op, r1, r2, l1, l2) ->
      fprintf fmt "%a %a %a;  goto %a, %a" print_mbbranch op Register.print r1 Register.print r2 Label.print l1
        Label.print l2
  | Ecall (x, n, l) -> fprintf fmt "call %s(%d);  goto %a" x n Label.print l
  | Egoto l -> fprintf fmt "goto %a" Label.print l
  | Ealloc_frame l -> fprintf fmt "alloc_frame;  goto %a" Label.print l
  | Edelete_frame l -> fprintf fmt "delete_frame;  goto %a" Label.print l
  | Eget_param (n, r, l) -> fprintf fmt "mov stackp(%d) %a --> %a" n Register.print r Label.print l
  | Epush_param (r, l) -> fprintf fmt "push %a;  goto %a" Register.print r Label.print l
  | Ereturn -> fprintf fmt "return"
...
let print_live_info fmt li =
  fprintf fmt "d={%a}@ u={%a}@ i={%a}@ o={%a}" print_set li.defs print_set li.uses print_set li.ins print_set li.outs

let print_graph fmt = visit (fun l i liveinfo -> fprintf fmt "%a: %a @\n" Label.print l print_instr i)
...
let print_ertlfile fmt p = List.iter (print_ertldef fmt p.liveness) p.funs
