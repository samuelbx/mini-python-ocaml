...
let rec type_expr ctx = function
  ...
  | Ebinop (op, e1, e2) ->
    ...
    | TEcst cst1, TEcst cst2 -> 
        ...
        | _, _, _ -> error "(typing) '%s' not supported between instances of '%s' and '%s'" (binop_to_string op) (ctype_to_string cst1) (ctype_to_string cst2))
    | TElist l1, TElist l2 -> 
      (match op with
  ...
  | Ecall (fn, args) ->
    ...
    | _ ->
        (if Hashtbl.mem function_table fn.id then
          ...
          List.iter (fun param -> Hashtbl.add ctx param.v_name { v_name = param.v_name; v_ofs = 0 }) callable.fn_params;
          if List.length callable.fn_params = List.length args then
            TEcall (callable, List.map (type_expr ctx) args)
          else
