...
let treat_ecall dest_reg fun_name rList lb =
  let lastLabel =
    if List.length rList > 6 then
      add_to_cfg (Emunop (Maddi (Int64.of_int ((List.length rList - 6) * 8)), Register.rsp, lb))
    else lb
  in
  ...
  let rec fill_recursif registerList index label =
    if index < 6 then
      ...
      | reg :: remain ->
          let newLabel = add_to_cfg (Embinop (Mmov, reg, List.nth Register.parameters index, label)) in
          fill_recursif remain (index + 1) newLabel
      | [] -> label
    else
      match registerList with
      | reg :: remain ->
          let newLabel = add_to_cfg (Epush_param (reg, label)) in
          fill_recursif remain (index + 1) newLabel
      | [] -> label
  in
  let secondLabel = fill_recursif rList 0 labelCall in
  let goto = Egoto secondLabel in
  goto
...
let ertl_fun fn =
  ...
  let rec get_args argList count label =
    if count < 6 then
      match argList with
      | reg :: remain ->
          let newLabel = add_to_cfg (Embinop (Mmov, List.nth Register.parameters count, reg, label)) in
          get_args remain (count + 1) newLabel
      | [] -> label
    else
      match argList with
      | reg :: remain ->
          let newLabel = add_to_cfg (Eget_param (((count - 6) * 8) + 16, reg, label)) in
          get_args remain (count + 1) newLabel
      | [] -> label
  in
  let first_arg_label = get_args fn.Rtltree.fun_formals 0 fn.Rtltree.fun_entry in
  let savedRegisterHsh = Hashtbl.create 16 in
  let rec save_register registerList label =
    match registerList with
    | registre :: remain ->
        let savedRegister = Register.fresh () in
        Hashtbl.add savedRegisterHsh registre savedRegister;
        let newLabel = add_to_cfg (Embinop (Mmov, registre, savedRegister, label)) in
        save_register remain newLabel
    | [] -> label
  in
  let first_calle_saved_lb = save_register Register.callee_saved first_arg_label in
  let alloc_lb = add_to_cfg (Ealloc_frame first_calle_saved_lb) in
  
  (* function output *)
  let return_lb = add_to_cfg Ereturn in
  let delete_frame_lb = add_to_cfg (Edelete_frame return_lb) in
  let rec restore_register registerList label =
    match registerList with
    | registre :: remain ->
        let savedRegister = Hashtbl.find savedRegisterHsh registre in
        let newLabel = add_to_cfg (Embinop (Mmov, savedRegister, registre, label)) in
        restore_register remain newLabel
    | [] -> label
  in
  ...
  graph := Label.M.add fn.Rtltree.fun_exit result_copy !graph;
  {
    ...
    fun_ctx = fn.Rtltree.fun_ctx;
    fun_entry = alloc_lb;
    fun_body = !graph;
  }

let rec ertl_funlist = function fn :: remain -> ertl_fun fn :: ertl_funlist remain | [] -> []

let file p =
  let funlist = ertl_funlist p.Rtltree.funs in
  let livenessMap = liveness !graph in
  { funs = funlist; liveness = livenessMap }
