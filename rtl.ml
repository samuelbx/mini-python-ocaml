...
and my_print_macro e ctx ld rd =
  let r_ret_useless = Register.fresh () in
      ...
      let lbl_3 =
        ...
        let l_putchar = add_to_cfg (Ecall (r_ret_useless, "printf", [r_fmt; r_char], l_incr_counter)) in
        ...
        let l_loadfmt = add_to_cfg(Econst(Cstring "%d", r_fmt, l_loadone)) in
        add_to_cfg (Econst(Cint 0L, r_counter, l_loadfmt))
      in
