...
let ltl_instr colors spilledNumber myinstr = match myinstr with 
  ...
  | Ertltree.Ealloc_frame (lb)->
    let last_label = if spilledNumber <> 0 then generate (Emunop(Maddi (Int64.of_int(- 8 * spilledNumber)), Reg(Register.rsp), lb)) else lb in
...
let rec ltl_funlist colors spilledNumber (funlist:Ertltree.ertldef list) = match funlist with 
